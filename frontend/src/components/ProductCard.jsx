    ...
    <div className="product-card" style={style} onClick={() => navigate(`/product/${product.id}`)}>
      <div className="product-card-img">
        ...
        <div style={{position:'absolute',top:8,left:8,display:'flex',flexDirection:'column',gap:4}}>
          ...
          {!inStock && <span style={{background:'rgba(0,0,0,0.6)',color:'var(--text-muted)',fontSize:'0.6rem',fontWeight:600,letterSpacing:'0.1em',textTransform:'uppercase',padding:'3px 8px',borderRadius:2}}>Sold Out</span>}
        </div>
      ...
      <div className="product-card-body">
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:8}}>
          <div>
            ...
            {origPrice && <span style={{fontSize:'0.72rem',color:'var(--text-muted)',textDecoration:'line-through',marginLeft:8}}>${Number(origPrice).toLocaleString()}</span>}
          </div>
          {inStock && (
